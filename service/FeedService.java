...
public class FeedService extends AbstractScheduledService {
    ...
    public Feed synchronize(String url) throws Exception {
        ...
        if (!articleToRemove.isEmpty()) {
            ...
            AppContext.getInstance(configDao).getEventBusManager().getAsyncEventBus().post(articleDeletedAsyncEvent);
        }
        ...
        if (feed == null) {
            ...
            ConfigDao configDao = new ConfigDao();
            AppContext.getInstance(configDao).getEventBusManager().getAsyncEventBus().post(faviconUpdateRequestedEvent);
        } else {
        ...
        if (!guidIn.isEmpty()) {
            ...
            for (ArticleDto currentArticle : currentArticleDtoList) {
                Article newArticle = articleMap.remove(currentArticle.getMetadata().getGuid());
                
                Article article = new Article();
                article.setPublicationDate(currentArticle.getDateInfo().getPublicationDate());
                ...
                if (!Strings.nullToEmpty(currentArticle.getContent().getTitle()).equals(Strings.nullToEmpty(article.getTitle())) ||
                        !Strings.nullToEmpty(currentArticle.getContent().getDescription()).equals(Strings.nullToEmpty(article.getDescription()))) {
            ...
            if (!articleUpdatedList.isEmpty()) {
                ...
                ConfigDao configDao = new ConfigDao();
                AppContext.getInstance(configDao).getEventBusManager().getAsyncEventBus().post(articleUpdatedAsyncEvent);
            }
        }
        ...
        if (!articleMap.isEmpty()) {
            ...
            ConfigDao configDao = new ConfigDao();
            AppContext.getInstance(configDao).getEventBusManager().getAsyncEventBus().post(articleCreatedAsyncEvent);
    ...
    private List<Article> getArticleToRemove(List<Article> articleList) {
        ...
        for (ArticleDto newerLocalArticle : newerLocalArticles) {
            if (!newerArticleGuids.contains(newerLocalArticle.getMetadata().getGuid()) && newerLocalArticle.getDateInfo().getCreateDate().after(dateMin)) {
